...
| `evaluate_permission` | read | Dry-run the pipeline for a hypothetical action ("could you unlock the door for my brother tomorrow?"), using the same flat `action` enum and parameters as `execute_household_action`: the would-be Decision with the rule and band, no side effects, no audit row beyond a `DRY_RUN` marker. |
...
| `ui://hirz/verification-card` | `assess_request_risk`, `verify_trusted_identity` | inline | One headline, up to three signals, verification status (`pending` → result) |
| `ui://hirz/doorbell-card` | `get_household_context` (scope `environment` or `security`) when a visitor context is active | inline | Snapshot, expected-visitor context, unlock request button (pipeline-gated) |
