...
|---|---|---|---|---|---|---|---|---|---|
| 1 | 2026-09-15 | Alexa+ MCP Toolkit docs | Read the add-on design guide (display modes, tools/schema/data) linked from the docs home | Followed the design-guide links from the MCP Toolkit overview page | The pages load | The linked URLs were not reachable | Minor | Designed from the functional-requirements page and the quickstart instead | Fix or remove the dead links on the overview page |
...
- AgentCore Policy temporal quotas (25 policies per engine, 3 operators per policy, 24-hour window) versus a constitution's `ask` classes; changing temporal policies returns 409 on open sessions.
- Ring sandbox: webhook signature details and synthetic-device event coverage.
